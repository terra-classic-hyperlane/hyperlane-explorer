...
export async function fetchDeliveryStatus(
  ...
  const destMailboxAddr = await getMailboxAddress(
    destName,
  ...
  const delivery = await checkIsMessageDelivered(
    message.msgId,
    message.destinationDomainId,
    destMailboxAddr,
    multiProvider,
  );
  const { isDelivered, blockNumber, transactionHash } = delivery;

  if (isDelivered) {
    const destName = multiProvider.tryGetChainName(message.destinationDomainId);
    const destProtocol = multiProvider.tryGetChainMetadata(
      message.destinationDomainId,
    )?.protocol;
    const isCosmosDestination = destProtocol === "cosmos";
    const isSealevelDestination = destProtocol === "sealevel";

    let txDetails: Awaited<ReturnType<typeof fetchTransactionDetails>>["tx"] =
      null;
    let blockTimestamp: number | null = null;
    let cosmosFrom: string | null = null;

    if (isSealevelDestination) {
      // The Sealevel check already resolved slot, time and fee payer; there is no
      // ethers provider for Solana to ask.
      blockTimestamp = delivery.timestamp ?? null;
      cosmosFrom = delivery.from ?? null;
    } else if (isCosmosDestination) {
      if (transactionHash) {
    ...
    const result: MessageDeliverySuccessResult = {
      ...
      deliveryTransaction: {
        ...
        to:
          txDetails?.to || (destName ? destMailboxAddr : constants.AddressZero),
        ...
        maxPriorityPerGas: toDecimalNumber(
          txDetails?.maxPriorityFeePerGas || 0,
        ),
      },
    };
    return result;
  } else {
    const originProtocol = multiProvider.tryGetChainMetadata(
    ...
    const canDebug =
      originProtocol === "ethereum" && destProtocol === "ethereum";
    ...
    const debugResult = await debugMessage(
      multiProvider,
      registry,
      overrideChainMetadata,
      message,
...
async function fetchTransactionDetails(
  ...
  logger.debug(
    `Searching for transaction details for ${txHash ?? `block ${blockNumber}`}`,
  );
...
async function fetchCosmosTransactionDetails(
  ...
  const meta = multiProvider.tryGetChainMetadata(domainId) as
    { restUrls?: Array<{ http: string }> } | null | undefined;
  ...
  try {
    ...
    const blockTimestamp = resp.timestamp
      ? Math.floor(new Date(resp.timestamp).getTime() / 1000)
      : null;

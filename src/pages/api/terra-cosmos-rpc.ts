...
export default async function handler(
  ...
  try {
    ...
    return res
      .status(502)
      .json({ error: `Failed to resolve RPCs: ${errMsg(e)}` });
  }
  if (!rpcUrls.length) {
    return res
      .status(404)
      .json({ error: `No RPC endpoints for chain ${chainName}` });
  }
  ...
  for (const rpcUrl of rpcUrls) {
    try {
      ...
      if (data && typeof data === "object" && "error" in data && data.error) {
        lastError = errMsg(
          data.error?.data ?? data.error?.message ?? data.error,
        );
        continue;
      }
